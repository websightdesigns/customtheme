				...
				<header class="page-header">
					<h1 class="page-title">
						<?php
							...
							elseif ( is_author() ) :
								printf( __( 'Posts by %s', 'customtheme' ), '<span class="vcard">' . get_the_author() . '</span>' );

							elseif ( is_day() ) :
								printf( __( 'Posts from %s', 'customtheme' ), '<span>' . get_the_date() . '</span>' );

							elseif ( is_month() ) :
								printf( __( 'Posts from %s', 'customtheme' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'customtheme' ) ) . '</span>' );

							elseif ( is_year() ) :
								printf( __( 'Posts from %s', 'customtheme' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'customtheme' ) ) . '</span>' );

							elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
								_e( 'Asides', 'customtheme' );

							elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
								_e( 'Galleries', 'customtheme' );

							elseif ( is_tax( 'post_format', 'post-format-image' ) ) :
								_e( 'Images', 'customtheme' );

							elseif ( is_tax( 'post_format', 'post-format-video' ) ) :
								_e( 'Videos', 'customtheme' );

							elseif ( is_tax( 'post_format', 'post-format-quote' ) ) :
								_e( 'Quotes', 'customtheme' );

							elseif ( is_tax( 'post_format', 'post-format-link' ) ) :
								_e( 'Links', 'customtheme' );

							elseif ( is_tax( 'post_format', 'post-format-status' ) ) :
								_e( 'Statuses', 'customtheme' );

							elseif ( is_tax( 'post_format', 'post-format-audio' ) ) :
								_e( 'Audios', 'customtheme' );

							elseif ( is_tax( 'post_format', 'post-format-chat' ) ) :
								_e( 'Chats', 'customtheme' );

							else :
								_e( 'Archives', 'customtheme' );
					...
					<ol class="breadcrumb">
						...
						<li class="active"><?php
							if( is_category() ) {
								...
								printf( __( 'Posts from %s', 'customtheme' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'customtheme' ) ) );
							} elseif( is_day() ) {
								printf( __( 'Posts from %s', 'customtheme' ), get_the_date() );
							} elseif( is_year() ) {
								printf( __( 'Posts from %s', 'customtheme' ), get_the_date( _x( 'Y', 'yearly archives date format', 'customtheme' ) ) );
							} elseif( is_author() ) {
								printf( __( 'Posts by %s', 'customtheme' ), get_the_author() );
							}
						?></li>
					</ol>
				<?php } ?>
